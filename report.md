...
In an attempt to optimise the code for the architecture of blue crystal, I switched to using the C compiler provided by Intel. The ICC is designed by Intel for Intel CPUs, where as CC and GCC are meant to be more general. This makes the code run faster as it is possible to exploit optimisations in the specific CPU.
